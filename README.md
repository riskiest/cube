...
定义函数 $`f_X(x)`$ 为棋子在上述投掷过程中踩过数轴上点 $`x`$ 的概率。需寻找满足条件的骰子 $`X`$，使得 $`\max_{x\in \mathbb{R}^{+}}f_X(x)`$ 最大。
...
3. 证明 $`X`$ 需要满足的初始约束集；
4. 对每组约束，对 $`x_6`$ 数值拆解，即尽可能枚举满足 $`x_6=\sum_{i=1}^{5}{k_ix_i}`$ 的所有表达，其中 $`k_i\in \mathbb{N}`$；
