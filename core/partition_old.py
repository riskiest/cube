...
class Partition:
    ...
    """
    def __init__(self, i_min: int,expr: BoolRef, breakdowns: List[Dict], solver: SMTSolver = None, id: Optional[str] = None):
        self.i_min = i_min
        self.expr = expr
    ...
    def order(self) -> None:
        ...
        self.breakdowns.sort(key=lambda item: tuple([item["coeffs"][f"x{i}"] for i in range(5, self.i_min-1, -1)]), 
                              reverse=True)
    ...
    def log_members(self) -> None:
        ...
        logger = self.get_logger()
        logger.info("=" * 40)
        logger.info(f"Partition {self.id} Members ({len(self.breakdowns)} total):")
        logger.info("=" * 40)
        for idx, member in enumerate(self.breakdowns, 1):
    ...
    def log_LHS(self, n=1, Breakdown=True, DEBUG=True) -> None:
        ...
        for i, member in enumerate(self.breakdowns, 1):
            LHS = member["LHS"]
    ...
    def gen_constraints(self, member_indices: List[int | tuple], n: int = 1) -> List[BoolRef]:
        ...
        for idx in member_indices:
            if isinstance(idx, int):
                ...
                member = self.breakdowns[member_id]
