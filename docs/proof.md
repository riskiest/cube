...
根据定理4，我们仅需考虑$t=x_6$的情况。也就是$x_6是X的最大值点$。
...
$f$可以定义在$\R^6$, 也可以扩展定义在$\R^5,\dots,\R^1$上，对于$\R^m$, 有
...
可知，当$[x_5>x_4, x_5\ne 2x_{1-4}]$时，$\theta(X)\le 0.02_6+0.0341344_6+0.11_6=0.2041344_6<\theta([1,2,3,4,5,8])$; 因此，为了使得能成为候选最优解，需要越是$[x_5=2x_{1-4}]$ ($x_5>x_4恒成立$)。
...
记作$\mathcal{F}_{K, i}(n),K=(k_5, k_4, \dots, k_{i+1})\in (\Z^{+})^{5-i},5\ge i\ge1$, $\mathcal{F}$关于$K$是对称函数，其中$n\ge-2$, 以下是$n$的含义
...
记$\mathcal{F}_{K, i}(n),K=(k_5, k_4, \dots, k_{i+1})\in (\Z^{+})^{5-i},5\ge i\ge1$, $\mathcal{F}$关于$K$是对称函数，其中$n\ge-2$, $i,n\in \Z$
